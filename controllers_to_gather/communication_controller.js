  ...
  if (phoneNo === null) {
    ...
    Alluser.getOne(query, function (err, item) {
      if (err) {
        ...
        req.outputs = {status: 1, msg: '不存在该用户!'}
        errorHandler.makeError(2, req.outputs)(req, res, next)
      } else {
        req.item = item
        return next()
...
exports.receiverIDbyPhone = function (req, res, next) {
  ...
  if (_messageType === 1) {               // 单聊获取用户userId
    ...
    req.outputs = {status: 1, msg: '请输入正确的信息类型!'}
    errorHandler.makeError(2, req.outputs)(req, res, next)
  }
}
...
exports.insertMember = function (req, res, next) {
  ...
  if (teamId === null) {
    ...
    req.outputs = {status: 1, msg: '请填写teamId!'}
    errorHandler.makeError(2, req.outputs)(req, res, next)
  }
  var query = {
    teamId: req.body.teamId
  ...
  Team.update(query, upObj, function (err, upmember) {
    ...
    if (upmember.nModified === 0 && upmember.n !== 0) {
      ...
      req.outputs = {status: 1, msg: '未成功修改！请检查是否成员已添加！'}
      errorHandler.makeError(2, req.outputs)(req, res, next)
    }
...
exports.removeMember = function (req, res, next) {
  ...
  Team.update(query, upObj, function (err, upmember) {
    ...
    if (upmember.n === 0 && upmember.nModified === 0) {
      ...
      req.outputs = {status: 1, msg: '未成功移除，请检查组是否存在！'}
      errorHandler.makeError(2, req.outputs)(req, res, next)
    }
    // upmember选中文件，但未更新文件，即团队编号匹配但团队成员不匹配
    if (upmember.n !== 0 && upmember.nModified === 0) {
      // return res.json({msg: '未成功移除，请检查成员是否存在！', status: 1})
      req.outputs = {status: 1, msg: '未成功移除，请检查成员是否存在！'}
      errorHandler.makeError(2, req.outputs)(req, res, next)
    }
...
exports.postCommunication = function (req, res, next) {
  var commmunicationData = {
    messageNo: req.newId,
    messageType: Number(req.body.messageType),
...
exports.getMassTargets = function (req, res, next) {
  ...
  DpRelation.getOne(query, function (err, doctorItem) {
    ...
    if (doctorItem === null) {
      ...
      req.outputs = {status: 1, msg: '暂无关注或主管的患者'}
      errorHandler.makeError(2, req.outputs)(req, res, next)
    } else {
      let targets = []
      let targetsUserId = []
      ...
      if (targets.length === 0) {
        ...
        req.outputs = {status: 1, msg: '无有效群发目标'}
        errorHandler.makeError(2, req.outputs)(req, res, next)
      } else {
