...
function messageSaveSend (data, url, socket) {
  ...
  data.msg.status = 'send_success'
    ...
    // save data
  var url = 'http://' + webEntry.domain + ':4060/communication/postCommunication'
  var jsondata = {
...
function sendToReceiver (messageType, receiver, sendBy, userAppServer, userWechatServer, data) {
  if (messageType == 1) {       // 单聊
    ...
    request({
            // url: 'http://' + webEntry.domain + ':4060/communication/getTeam?teamId=' + data.msg.teamId + '?token=' + req.query.token || req.body.token,
      url: 'http://' + webEntry.domain + ':4060/communication/getTeam?teamId=' + data.msg.teamId,
      method: 'GET',
      json: true
    }, function (err, response) {
      if (err) {
                  ...
                  'data': {
                    ...
                    'remark': {
                        ...
                        'color': '#173177'
                            ...
                            // groupSend(data);
              request({
                            // url: 'http://'+ webEntry.domain +':4060/wechat/messageTemplate' + '?token=' + req.query.token || req.body.token,
                url: 'http://' + webEntry.domain + ':4060/wechat/messageTemplate',
                method: 'POST',
                body: template,
                json: true
...
exports.chat = function (io, socket) {
  ...
  socket.on('message', function (data) {
    ...
    var url = 'http://' + webEntry.domain + ':4060/wechat/download'
